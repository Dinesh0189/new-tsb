...
let threeJS = { scene: null, camera: null, renderer: null, animator: null, objects: {}, mouse: { x: -100, y: -100 } };
...
function initSoundManager() {
    if (typeof Tone === 'undefined') {
        // Fallback: disable sounds gracefully
...
// 1) Add above bindUniversalEventListeners
function getTabSequence() {
    return Array.from(document.querySelectorAll('.nav-btn[data-tab]'))
      .map(btn => btn.dataset.tab);
  }
  function navigateRelative(delta) {
    const tabs = getTabSequence();
    if (!tabs.length) return;
    const current = document.querySelector('.nav-btn.active')?.dataset.tab || tabs[0];
    const idx = Math.max(0, tabs.indexOf(current));
    const nextIdx = (idx + delta + tabs.length) % tabs.length;
    const slideFrom = delta > 0 ? 'right' : 'left';
    window.changeTab(tabs[nextIdx], false, slideFrom);
  }
  function initMobileSwipeNavigation() {
    ...
    const threshold = 50, restraint = 80, allowedTime = 500;
    ...
    area.addEventListener('touchend', (e) => {
      ...
      const elapsed = Date.now() - startTime; swiping = false;
      if (elapsed <= allowedTime && Math.abs(distX) >= threshold && Math.abs(distY) <= restraint) {
        if (distX < 0) navigateRelative(+1); else navigateRelative(-1);
      }
    }, { passive: true });
  }
...
function bindUniversalEventListeners() {
    ...
    // Existing delegated listeners remain as-is
    ...
    on('add-exercise-btn', 'click', () => addExerciseField());
    const typeSel = document.getElementById('schedule-event-type');
    if (typeSel) {
    ...
    document.querySelectorAll('.modal-overlay').forEach(overlay => {
        overlay.addEventListener('click', (e) => {
            if (e.target === overlay) closeModal(overlay.closest('.modal-container').id);
        });
    });

    window.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            document.querySelectorAll('.modal-container.visible').forEach(m => closeModal(m.id));
        }
    });
...
// --- TAB/VIEW MANAGEMENT ---
// 3) Replace the whole changeTab definition with this
window.changeTab = (tabName, fromListener = false, slideFrom = null) => {
    ...
    main.classList.remove('slide-in-from-left','slide-in-from-right');
    main.innerHTML = '';
    ...
    if (slideFrom === 'left') main.classList.add('slide-in-from-left');
    else if (slideFrom === 'right') main.classList.add('slide-in-from-right');
...
function handleThreeJSMouseMove(event) {
    if (typeof THREE !== 'undefined') {
        if (!(threeJS.mouse instanceof THREE.Vector2)) {
            threeJS.mouse = new THREE.Vector2();
        }
        ...
        switch (action.toLowerCase()) {
            ...
            case 'getschedule': {
                ...
                if (schedule) {
                    ...
                    confirmation = events.length > 0
                        ? `On ${schedule.day}, you have ${events.join(', ')}.`
                        : `You have no events scheduled for ${schedule.day}.`;
                } else {
                    throw new Error(`No schedule found for ${payload.day}.`);
                }
                break;
            }
            default:
